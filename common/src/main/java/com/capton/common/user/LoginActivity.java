...
# 500 :请求方法错误
    * */
    ...
    private void afterLoginSuccess(boolean islogined,User user) {
        ...
        System.out.println("LoginActivity.afterLoginSuccess "+user.getUsername());
    ...
    private void tologin(){
        final ProgressDialog dialog = ProgressDialog.show(this,"","登录中");
        String baseUrl = getString(R.string.baseurl);
        String loginUrl = baseUrl+getString(R.string.login);
        OkGo.<String>post(loginUrl)
    ...
    private void handleResult(Response<String> response) {
        try {
            LoginResultBean resultBean = (LoginResultBean) JsonUtil.strToObject(response.body(), LoginResultBean.class);
    ...
    private void resetPsw() {
        final ProgressDialog dialog = ProgressDialog.show(this,"","修改密码中");
        String baseUrl = getString(R.string.baseurl);
        String resetPswUrl = baseUrl+getString(R.string.resetPsw)+binding.user.getText().toString();
        OkGo.<String>post(resetPswUrl)
                .params("adminPsw",getString(R.string.adminPsw))
                .params("newPsw",binding.psw.getText().toString())
                .execute(new StringCallback() {
                    @Override
                    public void onSuccess(Response<String> response) {
                        try {
